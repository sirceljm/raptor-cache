...
function DiskCache(config, settingsName, cacheName) {
    ...
    var subDir = cacheName ? safeFilename(settingsName) + '/' + safeFilename(cacheName) : safeFilename(settingsName);
    this._file = nodePath.join(dir, subDir, 'cache.json');
...
DiskCache.prototype = {
    ...
    _write: function() {
        ...
        function doWrite() {
            logger.debug('Persisting cache to file "' + _this._file + '"...');
            ...
            writePromise.then(
                function fulfilled() {
                    logger.debug('Written to disk: ' + _this._file);
                },
