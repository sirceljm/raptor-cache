...
function readFromDisk(store, callback) {
    ...
    function done(err) {
        ...
        if (debugEnabled) {
            logger.debug('readFromDisk() - done.', err ? 'With error: ' + err : '(no errors)');
        }
...
function DiskStore(config) {
    ...
    logger.info('Created DiskStore cache ' + getStoreName(this) + ' at ' + dir);
}
...
DiskStore.prototype = {
    ...
    flush: function(callback) {
        ...
        if (debugEnabled) {
            logger.debug('Cache ' + getStoreName(this) + ' flush() cache keys: ', Object.keys(this.cache));
        }
        ...
        function done(err) {
            if (debugEnabled) {
                logger.debug('Cache ' + (this.name || '(unnamed)') + ' flush() - done.', err ? 'Error: ' + err : '');
            }
        ...
        out.on('close', function() { // The flush is completed when the file is closed
            if (debugEnabled) {
                logger.debug('Cache ' + getStoreName(_this) + ' flush completed to file ' + tempFile);
            }
            ...
            fs.unlink(file, function() {
                fs.rename(tempFile, file, function(err) {
                    ...
                    logger.debug('Cache ' + getStoreName(_this) + ' renamed temp file to ' + file);
        ...
        function writeInlineValue(key, cacheEntry) {
            ...
            if (value !== undefined) {
                ...
                if (typeof value !== 'string' && !(value instanceof Buffer)) {
                    if (!serialize) {
                        throw new Error('Error writing value for cache ' + getStoreName(this) + '. Serializer is required for non-String/Buffer values. Cannot write ' + key + ' with value of type ' + (typeof value));
                    }
                ...
                if (debugEnabled) {
                    logger.debug('writeInlineValue: valueLen: ', value.length);
                }
                ...
                inStream.pipe(through(function write(data) {
                    ...
                    function end () { //optional
                        ...
                        logger.debug('writeInlineValue: valueLen: ', valueBuffer.length);
        ...
        function continueWriting() {
            logger.debug('continueWriting(), i:', i, numKeys, 'bufferAvailable:', bufferAvailable, 'readyForNext:', readyForNext);
            ...
            while (i < numKeys && bufferAvailable && readyForNext) {
                ...
                if (debugEnabled) {
                    logger.debug('Writing #' + (numWritten + 1) + ', ' + (i+1) + ' of ' + numKeys, ', key: ', key);
                }
                ...
                if (!isObjectEmpty(meta)) {
                    ...
                    if (debugEnabled) {
                        logger.debug('Meta (length: ' + metaBuffer.length + '): ' + metaJson);
                    }
